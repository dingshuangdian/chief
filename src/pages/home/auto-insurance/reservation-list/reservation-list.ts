...
export class ReservationListPage {
    ...
    this.webSites.httpPost('payOrder', param)
      .subscribe(res => {
        this.csModal.showAlert('提交成功，等待客服处理', () => {
          ...
          // });
          // popover.present();
          this.navCtrl.push(CarInsProgressPage, { num: 2 });
  ...
  // 门店
  selectShop() {
    if (this.storeInfo.companyAddress) {
      let alert = this.alertCtrl.create({
        title: '提示',
        message: '您是否将门店(' + this.storeInfo.companyAddress + ')设置为保单收件地址？',
        buttons: [
          {
            text: '取消',
            role: 'cancel',
      ...
      this.csModal.showAlert('门店地址为空，请手动输入');
    }
  }
